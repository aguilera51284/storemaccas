...
export const cartPriceTotal = cartItems => {
  return cartItems.reduce((acc, cur) => {
    return acc + cur.totalPrice * cur.quantity
  }, 0);
}
...
export const pickCart = (product) => {
  const { id, attributes } = product;
  const { name, slug, thumbnail, price, code, priceTax, hasDiscount, totalPriceTax, equivalence, productBrand, year, carBrand } = attributes;
  const image = thumbnail.data.attributes.url;

  return { id, name, slug, image, price, code, equivalence, productBrand, year, carBrand, priceTax, hasDiscount, totalPriceTax }
}


// detect if render is SSR or SPA
export const isSSR = () => {
 ...
 * @return {array}
 */
export const serializeCartList = (cartList) => cartList.map(item => ({
